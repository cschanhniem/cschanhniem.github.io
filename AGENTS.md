...
- `AN 4.1-10` marks a real editorial transition. After the long matrix work of `AN 3.183-352`, do not keep serial peyyāla rhythm by habit. These are individual discourses again, and each route needs its own argumentative body and cadence.
- Within that opening `AN 4` block, `an4.6-10` should be handled as five distinct shapes: the relation between learning and grasping the point, the four classes who grace the Saṅgha, the Buddha's four self-assurances, the four occasions for craving to arise, and the four yokes plus the four unyokings. Do not flatten them into short doctrinal notes.
- `AN 4.11-20` continues the same individualized-discourse logic. `an4.11-16` are training texts, posture, ethics, right effort, restraint, relative primacy, and subtlety. `an4.17-20` pivot into just and unjust decision-making, so the key vocabulary around favoritism, hostility, delusion, and fear must stay stable across the whole mini-cluster.
- `AN 4.21-30` is another full-discourse band, but internally split in two. `an4.21-26` are high doctrinal and communal-shape texts: reverence for Dhamma after awakening, true seniority by qualities rather than age, the Tathāgata and the world, the Kāḷaka discourse on knowing without conceiving, the real purpose of the holy life, and the contrast between deceptive versus genuine monastics. `an4.27-30` then gather into one ascetic-contentment cluster: trifling requisites, noble traditions, the footprints of the Dhamma, and their defense before wanderers. Do not flatten the first half into aphorisms or the second half into a single stock paragraph.
- `AN 4.31-40` shifts again. `an4.31-34` form a fourfold-support cluster: conditions for prosperity, the four saṅgahavatthu, the lion’s roar of the Tathāgata, and four supreme confidences. `an4.35-38` return to individual doctrinal bodies and must keep their internal architecture intact. `an4.39-40` are paired sacrifice discourses: both reject violent sacrifice, but `Udāyī` leans harder into timely, well-prepared, well-directed offering to worthy recipients. Keep the pair distinct.
- `AN 4.41-50` is another mixed band. `an4.41-42` are compact instructional discourses and should stay exact. `an4.43-44` form a mirrored pair on valuing anger, denigration, gains, and honor over true dhamma. `an4.45-46` are a reporting pair on Rohitassa and the end of the world, where the second discourse must preserve the retold frame to the Saṅgha. `an4.47-50` then widen into contrast, eloquence, cognitive inversion, and corruption. Keep the doctrinal temperature rising across that arc.
- Do not trust the Minh Châu shell alone for `an4.17-19`. That part of the block compresses too aggressively. Restore each route from the English source so the prejudice triad reads as three distinct discourses again: the bad drift, the good drift, and the paired comparison.
- `AN 2.230-279` closes the ethical-valuation matrix built on the same five pairs from `AN 2.180-229`. Keep the ten predicates visibly ordered as unskillful and skillful, blameworthy and blameless, suffering-outcome and happiness-outcome, painful-result and pleasant-result, hurtful and not hurtful. Do not let fifty child routes collapse back into one grouped shell label.
- After `AN 2.230-279`, the next active block `AN 2.280-309` is no longer a moral-pair peyyāla. It pivots into Vinaya institutions and procedures while preserving the same twofold rationale, welfare of the Saṅgha and protection of the training. Reset the semantic frame before writing that tranche.
- `AN 2.280-309` is now complete in the manual layer. Treat it as a structural hinge inside `AN 2`: first one seed discourse on why rules are laid down, then twenty-nine Vinaya forms carrying the same twofold rationale. Keep those routes full-bodied, not shorthand.
- After `AN 2.280-309`, the active lane shifts again. `AN 2.310-479` begins the greed peyyāla, where the dominant frame is no longer Vinaya procedure but the paired work of knowing, fully understanding, abandoning, fading, and relinquishing greed and related defilements through two practices. Do not carry Vinaya diction forward.
- `AN 2.310-339` is now complete in the manual layer as three full ten-step ladders: greed, hate, and delusion. Keep the predicate order stable across each cluster: thắng tri, biến tri, biến tận, đoạn tận, trừ diệt, hoại diệt, ly, đoạn diệt, xả bỏ, từ bỏ.
- After `AN 2.339`, the next active turn is `AN 2.340+`, beginning with anger and then continuing through the same ladder over the remaining defilements. Preserve the ladder; only the affliction changes.
- `AN 2.340-369` is now complete in the manual layer for anger, acrimony, and disdain. Keep reading this zone as one uninterrupted defilement-ladder, not as separate thematic essays.
- After `AN 2.369`, the next defilements in order are contempt, jealousy, and stinginess. Do not improvise the sequence; follow the canonical chain already present in the grouped source.
...
- The prompt pack now includes a dedicated `Resume Lane Prompt`. When a new agent must continue a running manual 2026 lane, the default entrypoint is `worklog-translate-2026.md`, then `AGENTS.md`, `SKILL.md`, and adjacent route files. Do not resume from audit guesses alone.
...
- A manual 2026 lane is only `resume-safe` when four surfaces agree: the new route files, the task log in `tasks/`, the updated `worklog-translate-2026.md`, and the post-batch audit coverage.
...
- `AN 2.77-86` is a causal-disassembly block. The whole force lies in naming the support on which unwholesome states arise, then showing that when that exact support is abandoned, the states do not occur. Keep `nimitta`, `nidāna`, `hetu`, `saṅkhāra`, `paccaya`, `rūpa`, `vedanā`, `saññā`, `viññāṇa`, and `saṅkhatārammaṇa` distinct. Do not smooth them into one repeated sentence with different nouns swapped invisibly.
- `AN 2.87-97` is a compact doctrinal catalog. The risk is not under-translation but flattening. Each route must keep its own doctrinal pair visible, `ceto-vimutti/paññā-vimutti`, `hiri/ottappa`, `dovacassatā/pāpamittatā`, `sovacassatā/kalyāṇamittatā`, and the two skills around offenses. Keep titles exact enough that a reader can scan the route list and know what is being taught.
- `AN 2.98-117` begins the explicit `Bālavagga` mirror. `an2.98-107` must read as sharp paired contrasts between `kẻ ngu` and `người hiền trí`, not as one generic warning about foolishness. Preserve the exact object of confusion in each pair: future burden and present burden, allowable and unallowable, offense and non-offense, Dhamma and not-Dhamma, training and not-training. Then let `an2.108-117` pivot cleanly into the second ladder, where the same confusions are rephrased by their consequence, lậu hoặc tăng hay không tăng. If titles blur the object, the reader loses the whole architecture of the block.
- Inside that same block, `an2.109-117` should sound heavier than `an2.98-107`, even when the wording stays brief. The source now judges the practical consequence of confusion. Do not title them as if they were merely epistemic corrections. Make sure the reader hears the force, these are the two whose taints do not grow, or do grow.
- `AN 2.118-129` is the first mixed-register block after `Bālavagga`. Keep its three internal turns explicit. `an2.118-122` are social and economic character types, hope, initiative, gratitude, contentment, hoarding, wasting. `an2.123-126` pivot into doctrinal causality, the conditions for greed, hate, wrong view, and right view. `an2.127-129` then compress into Vinaya classification pairs. Do not let the middle doctrinal quartet inherit the warm tone of the opening social sayings, and do not let the final three lose their technical precision as offense categories.
- `AN 2.130-140` opens with aspiration but should not read like ambition. `an2.130-133` are exemplar-facing routes for four audiences, monks, nuns, male lay followers, female lay followers. Keep the tone reverent and directional. `an2.134-137` are heavier two-part discourses about praise and blame, confidence and distrust, and right or wrong conduct toward parents, the Tathāgata, and the Tathāgata’s disciples. These need full bodies, not compressed shells. `an2.138-140` then contract sharply into terse doctrinal pairs. Let the prose contract with them.
- `AN 2.141-150` is a compact giving-cluster built on verbal variation, not doctrinal drift. The real task is to keep each route distinct while preserving the same luminous hierarchy: material aid is good, but giving or extending the Dhamma is better. Do not collapse the ten child routes back into one generic `Bố Thí` article. Let the diction track the exact verb, giving, offering, relinquishing, surrendering, enjoying, sharing, including, supporting, sympathizing, while keeping cadence clean enough to read aloud.
- `AN 2.151-162` continues the same paired architecture, but the lexical field shifts from giving to reception, seeking, honoring, flourishing, and expansion. Keep each child route exact. `an2.151-152` are about welcome and hospitality. `an2.153-155` intensify the act of seeking. `an2.156-157` move to worship and guest-service. `an2.158-162` turn toward success, growth, treasure, accumulation, and increase. Do not let the Minh Châu grouped shell flatten these into twelve near-synonyms.
- `AN 2.163-179` is the first real training block in Book Two after the gift and reception clusters. Keep its internal ladder visible. `an2.163` begins with skill in entering and leaving attainment. `an2.164-167` gather soft human virtues, integrity, gentleness, patience, camaraderie, harmlessness, purity. `an2.168-169` are the dark-bright pair on sense restraint and moderation in eating. `an2.170-172` turn to contemplative strengths, reflection and cultivation, mindfulness and samadhi, serenity and insight. `an2.173-176` are the ethical-view ladder. `an2.177-179` close with urgency and lucid awareness. Do not flatten the whole block into a bland list of virtues.
- `AN 2.180-229` is a peyyāla engine, but it still has an internal matrix. Do not attempt the whole fifty-route span as one undifferentiated mass. Work it in clean subclusters. `an2.180-184` are the dark pairs, `an2.185-189` the bright counterpairs, `an2.190-194` suffering in the present, `an2.195-199` happy abiding, `an2.200-204` decline for the trainee, `an2.205-209` non-decline, `an2.210-214` hellward destiny, `an2.215-219` heavenward destiny, `an2.220-224` bad rebirth, `an2.225-229` good rebirth. Keep route identity exact even though the grouped sources are terse.
- `AN 2.230-279` is the next peyyāla matrix after anger, but now the governing labels shift from moral pairs into blunt value judgments such as unwholesome or wholesome, blameworthy or blameless, suffering-bringing or happiness-bringing, and so on. Do not carry over the exact diction of the anger block. Carry over only the matrix discipline.
...
- `AN 2.430-479` closes the whole peyyāla ladder of `AN 2`. Keep `sārambha`, `māna`, `atimāna`, `mada`, and `pamāda` distinct as `cuồng nhiệt`, `mạn`, `quá mạn`, `kiêu căng`, and `phóng dật`. Once `an2.479` is done, the strict sequential lane must hand off cleanly to `an3.1`.
...
- `AN 3.41-51` shifts from concise faith-discourses into a stronger public-ethical cluster. Keep `an3.41-49` lean and exact, because their force lies in clean classification, not ornament. Then let `an3.50` keep the full criminal simile with its three supports, and let `an3.51` breathe as an old-age warning discourse with dialogue and verse intact. After `an3.51`, continue strictly to `an3.52`.
- `AN 3.52-63` keeps widening the frame. `an3.52-55` are still dialogue-driven and must stay precise about what counts as `visible here and now`. `an3.56-61` pivot into social decline, giving, harsh asceticism, and wrong views, so do not soften the polemical edge. `an3.62-63` broaden again into shared danger and the Buddha's own `three great beds`, which should read spacious and vivid rather than schematic. After `an3.63`, continue strictly to `an3.64`.
- `AN 3.64-74` is a public-discourse cluster. `an3.64` and `an3.74` need visible argumentative pressure, not flattened summary prose. `an3.65-66` carry the anti-credulity and anti-defilement line, so preserve the practical testing tone. `an3.71-73` return to doctrinal clarification through dialogue and should stay crisp, not grandiose. After `an3.74`, continue strictly to `an3.75`.
- `AN 3.75-85` is a mixed but coherent bridge block. `an3.75` is pastoral and exhortative, so let the threefold establishment of confidence land as advice for real relationships, not as a doctrinal checklist. `an3.76-77` are paired teachings on future existence and must keep the difference between consciousness as seed and intention as initiating thrust. `an3.78-80` shift into dialogue, ethical discrimination, and cosmic scale, so keep the prose supple rather than formulaic. `an3.81-85` then gather around the three trainings, but each route must still keep its own image or function: injunction, donkey simile, farmer simile, Vajji monk case, and definition of the trainee. After `an3.85`, continue strictly to `an3.86`.
- `AN 3.86-95` begins with a dense training matrix and then changes register twice. `an3.86-88` must preserve the graded logic of partial and full practice, without collapsing the different attainment ladders into generic stream-entry prose. `an3.89-90` are a deliberate pair where `higher wisdom` changes from four noble truth knowledge to direct taintless liberation, so keep that contrast explicit. `an3.91` must retain confession, correction, and the Buddha's institutional discernment. `an3.92-95` reopen into simile and communal training, so let the prose breathe while keeping the sequence from effort to liberation exact. After `an3.95`, continue strictly to `an3.96`.
- `AN 3.96-107` is another hinge block. `an3.96-98` are a matched triad and must keep the royal thoroughbred frame intact while letting the final criterion escalate from right understanding to non-returning to arahantship. `an3.99-102` are dense simile discourses and should read vividly, not as compressed doctrinal notes. `an3.103-106` revolve around gratification, drawback, and escape, so keep those three terms stable and audible across the cluster. `an3.107` should stay brief and cutting. Do not decorate it. After `an3.107`, continue strictly to `an3.108`.
- `AN 3.108-118` starts with terse disciplinary sayings, then widens into root-psychology, karmic danger, rare persons, immaterial attainments, and the paired `failure/accomplishment` discourses. Keep the first three routes austere. `an3.111-112` must not be merged in sense, because one speaks of roots and the other of the cognitive rise of bondage. `an3.117-118` are paired on ethics, mind, and view, but `an3.118` must keep the dice simile as the interpretive edge. After `an3.118`, continue strictly to `an3.119`.
- `AN 3.119-129` is another reset block. `an3.119-122` look formulaic but are not interchangeable, because the organizing triads shift from action and livelihood, to purity, to sagehood. `an3.123-127` must keep their narrative grain: food, Saṅgha conflict, doctrinal confidence, public rebuke, and celestial gratitude. `an3.128` should stay sharp and unpleasant in the right way, since the flies simile is meant to sting. `an3.129` is brief and diagnostic, so keep it exact and unsentimental. After `an3.129`, continue strictly to `an3.130`.
- `AN 3.130-141` moves through a subtle but important turn. `an3.130` is not a praise piece for meditation powers but a surgical diagnosis of conceit, restlessness, and remorse. `an3.131-139` then form a compact doctrinal and social cluster, so keep each triad distinct even when the body is short. `an3.140-141` must read as an intentional pair: first the untrained speed of insight without maturity, then the noble thoroughbred whose speed is joined with beauty and proportion. After `an3.141`, continue strictly to `an3.142`.
- `AN 3.142-151` begins with a final culmination image, then narrows into terse doctrinal declarations. `an3.142` must sound like a completion of the horse sequence, not a duplicate of the prior pair. `an3.143-145` are short but elevated, so keep their compression without draining their force. `an3.146-151` then build a deliberate matrix around body, speech, and mind, shifting from unwholesome to blameworthy and from damaged to intact. Preserve each distinction cleanly. After `an3.151`, continue strictly to `an3.152`.
- `AN 3.152-155` is a mixed hinge. `an3.152` must keep the three-path structure clear, especially the difference between indulgent sinking, self-mortifying extremity, and the cultivated middle way. `an3.153-154` are brief and must stay clean. `an3.155` should keep its blessing cadence and oral lift, because the verse-like close is doing real work.
- `AN 3.156-162` is recoverable despite the malformed Minh Châu child files. Use the grouped English Bilara file `an3.156-162_en_sujato.json`, which carries exact child-prefixed keys for every route, and use the grouped Minh Châu shell only as the Vietnamese doctrinal cross-check for the repeated structure and the ordered middle-way sets. Do not skip this block, and do not invent new doctrine. Expand the repeated first two practices in full so the manual layer stays route-complete. After `an3.162`, continue strictly to `an3.163`.
- `AN 3.163-182` is a ten-pair `kammapatha` block. Do not treat it as twenty unrelated micro-suttas. Every route keeps the same threefold karmic structure, self-performance, urging others, and approving, while the pair logic flips the destination from hell to heaven. Recover route titles from the actual deed pairs, not from the broken grouped metadata label. After `an3.182`, continue strictly to `an3.183`.
- `AN 3.183-352` is a `Rāgapeyyāla` matrix, not a normal child-prefixed grouped block. The grouped English source compresses one doctrinal grid instead of exposing route IDs directly. Recover route identity by holding one defilement constant while walking the ordered ten operations, then move to the next defilement cluster. `an3.183-192` are all about greed. Do not rename the three samādhis loosely, and do not flatten the operation verbs into one generic line about abandoning defilements.
- `AN 3.193-202` are the second ten routes inside that same `Rāgapeyyāla` matrix. Keep the exact same ten-operation ladder used for greed, but switch the object to hatred: thắng tri, biến tri, biến diệt, đoạn tận, diệt tận, trừ diệt, ly, đoạn diệt, trừ khử, từ bỏ sân. The doctrinal movement is serial, not inventive. Do not paraphrase these into ten vague synonyms.
- `AN 3.203-212` are the third matrix slice and move from hatred to delusion. Keep the same ten operations again, now applied to `si`. This lane should read like a doctrinal table unfolding one object at a time, not like separate loosely themed aphorisms.
- `AN 3.213-222` are the fourth slice and move from delusion to anger. Keep the verb ladder untouched and change only the object to `phẫn nộ`. If titles or prose start varying too much here, the matrix logic becomes harder to audit later.
- `AN 3.223-232` continue the same matrix with acrimony. Keep the same ten operations and move the object to `hiềm hận`. Use one stable Vietnamese label across the tranche so future passes can still recognize the serial table.
- `AN 3.233-242` continue the same matrix with disdain. Keep the same ten operations and move the object to `khinh miệt`. This is another place where terminology drift can quietly break the table, so keep the Vietnamese label stable through all ten routes.
- `AN 3.243-252` continue the same matrix with contempt. Keep the same ten operations and move the object to `miệt thị`. Distinguish it from the prior `khinh miệt` slice by keeping the chosen label steady within the tranche and recording it in the worklog.
- `AN 3.253-262` continue the same matrix with jealousy. Keep the same ten operations and move the object to `ganh tỵ`. This is another tranche where casual synonym drift can blur the serial structure, so keep the term stable from title to title.
- `AN 3.263-272` continue the same matrix with stinginess. Keep the same ten operations and move the object to `xan tham`. This tranche should stay lean and serial, because the doctrinal signal here is the fixed ladder, not ornamental variation.
- `AN 3.273-282` continue the same matrix with deceitfulness. Keep the same ten operations and move the object to `giả dối`. Use that label consistently through the tranche, because the Minh Châu grouped shell already preserves it and later agents will need the same anchor when the matrix moves on to deviousness.
- `AN 3.283-292` continue the same matrix with deviousness. Keep the same ten operations and move the object to `man trá`. Hold that label steady, because this slice sits right after `giả dối`, and any synonym drift here will blur the distinction between plain deceit and scheming crookedness.
- `AN 3.293-302` continue the same matrix with obstinacy. Keep the same ten operations and move the object to `ngoan cố`. Let the wording stay firm and plain, because softening it into a generic stubborn mood will flatten the doctrinal edge of the tranche.
- `AN 3.303-312` continue the same matrix with aggression. Keep the same ten operations and move the object to `bồng bột nông nổi`. Do not shorten it casually, because the grouped Minh Châu shell preserves the fuller phrase and the matrix needs that source-linked continuity.
- `AN 3.313-322` continue the same matrix with conceit. Keep the same ten operations and move the object to `mạn`. Keep that label short and exact, because the next slice is `tăng thượng mạn`, and blurring the two will break the doctrinal staircase.
- `AN 3.323-332` continue the same matrix with arrogance. Keep the same ten operations and move the object to `tăng thượng mạn`. Protect its separation from the prior `mạn` slice, because the doctrinal gain here depends on preserving a higher-order inflation, not just repeating pride in another costume.
- `AN 3.333-342` continue the same matrix with vanity. Keep the same ten operations and move the object to `kiêu`. Do not let it slide back into either `mạn` or `tăng thượng mạn`, because this slice needs a cleaner worldly self-exaltation, not another replay of the prior two conceit tiers.
- `AN 3.343-352` continue the same matrix with negligence. Keep the same ten operations and move the object to `phóng dật`. This final slice should sound like heedlessness as a doctrinal fault line, not just casual carelessness, because it closes the whole serial ladder from greed through the conceit cluster into collapse of vigilance.
- `AN 4.1-10` leaves the `Rāgapeyyāla` matrix entirely and re-enters full individual discourses. Do not carry over the serial-peyyāla cadence from `AN 3.183-352`. `an4.1-5` should read as opening statements of the Book of Fours: doctrinal recollection, fallen versus secure, broken judgment, broken conduct, and the four kinds of persons moving with or against the stream. Each route needs its own argumentative shape.
