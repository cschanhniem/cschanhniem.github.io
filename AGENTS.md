...
- `AN 3.152-155` is a mixed hinge. `an3.152` must keep the three-path structure clear, especially the difference between indulgent sinking, self-mortifying extremity, and the cultivated middle way. `an3.153-154` are brief and must stay clean. `an3.155` should keep its blessing cadence and oral lift, because the verse-like close is doing real work. After `an3.155`, continue strictly to `an3.156`, but note that local source currently collapses `an3.156-162` into a grouped shell and needs source-shape verification before route-level authoring.
